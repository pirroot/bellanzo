...
import { ImageIcon, Loader2, Package, Plus, Save, Tag, Trash2, X, Wrench } from 'lucide-react';
...
export default function ProductsAdmin() {
  ...
  const [sparePartsForProduct, setSparePartsForProduct] = useState<Product[]>([]);
  ...
  const loadSpareParts = useCallback(async (productId: number) => {
    try {
      const data = await apiFetch<{ results: Product[] }>(
        `/products/?main_product=${productId}&is_spare_part=true`,
        { auth: true }
      );
      setSparePartsForProduct(data?.results || []);
    } catch {
      setSparePartsForProduct([]);
    }
  }, []);
    ...
    try {
      ...
      await Promise.all(
        items
          ...
          .map((i) => apiFetch(`/${endpoint}/${i.id}/`, { method: 'DELETE', auth: true }))
      );
      await load();
    } catch (err) {
  ...
  const openProductDrawer = useCallback(
    (product: Partial<Product> | null, isNew: boolean) => {
      setSelectedProduct(product);
      setIsNewProduct(isNew);
      if (product?.id) {
        loadSpareParts(product.id);
      } else {
        setSparePartsForProduct([]);
      }
    },
    ...
    <div>
      ...
      {loading ? (
        ...
        products.length === 0 ? (
          <div className="text-center py-20 bg-white rounded-3xl border border-line">
            ...
            <button
              onClick={() => openProductDrawer({ ...EMPTY_PRODUCT }, true)}
              className="btn btn-primary text-sm"
            >
                  ...
                  return (
                    ...
                    >
                      ...
                      <td
                        ...
                        onClick={() => openProductDrawer({ ...p }, false)}
                      >
                      ...
                      <td className="p-3" onClick={() => openProductDrawer({ ...p }, false)}>
                        <span
      ...
      {selectedProduct !== null && (
        <ProductDrawer
          ...
          categories={cats}
          products={products}
          spareParts={sparePartsForProduct}
          onClose={() => {
          ...
          onEditProduct={openProductDrawer}
        />
      )}
      {selectedCat !== null && (
        <CategoryDrawer
...
function ProductDrawer({
  ...
  onClose: () => void;
  onSaved: () => void;
  onEditProduct: (product: Partial<Product>, isNew: boolean) => void;
}) {
  ...
  async function save() {
    ...
    try {
      ...
      if (form.main_product) fd.append('main_product', String(form.main_product));
      fd.append(
        'features',
        JSON.stringify(
          featuresText
  ...
  async function deleteSparePart(id: number) {
    if (!confirm('آیا از حذف این قطعه یدکی مطمئن هستید؟')) return;
    try {
      await apiFetch(`/products/${id}/`, { method: 'DELETE', auth: true });
      onSaved();
    } catch (err) {
      alert(err instanceof Error ? err.message : 'خطا در حذف قطعه');
    }
  }

  const mainProductOptions = products.filter(
    (p) => !p.is_spare_part && (form.id ? p.id !== form.id : true)
  );
  ...
  return (
    <div className="fixed inset-0 z-[60]">
      ...
      <div className="absolute inset-y-0 right-0 w-full sm:max-w-lg bg-white overflow-y-auto p-5 sm:p-7 shadow-2xl flex flex-col gap-5">
        ...
        {form.is_spare_part && (
          <div>
            <label className="block text-sm font-bold text-ink-soft mb-2">محصول اصلی</label>
            <select
              value={form.main_product || ''}
              onChange={(e) => set('main_product', e.target.value ? Number(e.target.value) : null)}
              className={field}
            >
              <option value="">بدون محصول اصلی</option>
            ...
            <p className="text-xs text-muted mt-1">محصولی که این قطعه برای آن است</p>
          </div>
        )}
        ...
        <label
          className={`flex items-center justify-between gap-3 rounded-2xl border-2 p-4 cursor-pointer transition-colors ${form.is_spare_part ? 'border-amber-400 bg-amber-50' : 'border-line bg-surface'}`}
        >
          <div>
        ...
        {form.is_spare_part && (
          <label
            className={`flex items-center justify-between gap-3 rounded-2xl border-2 p-4 cursor-pointer transition-colors ${form.show_in_products !== false ? 'border-blue-400 bg-blue-50' : 'border-line bg-surface'}`}
          >
            <div>
        ...
        {!isNew && !form.is_spare_part && form.id && (
          <div className="border-t border-line pt-4 mt-2">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <Wrench size={18} className="text-amber-500" />
                <span className="font-bold text-ink text-sm">قطعات یدکی</span>
              ...
              <button
                ...
                className="flex items-center gap-1.5 text-sm font-bold text-brand hover:text-brand-dark transition-colors bg-brand-soft hover:bg-brand/20 px-3 py-1.5 rounded-xl"
              >
                <Plus size={16} />
                افزودن قطعه جدید
              </button>
            ...
            {spareParts.length === 0 ? (
              <div className="bg-surface rounded-2xl border-2 border-dashed border-line p-6 text-center">
                <Wrench size={32} className="mx-auto text-line mb-2" />
                <p className="text-sm text-muted">هیچ قطعه یدکی برای این محصول ثبت نشده است.</p>
                <button
                  type="button"
                  onClick={() => setShowAddSparePart(true)}
                  className="mt-2 text-sm text-brand hover:underline font-bold"
                >
                  اولین قطعه را اضافه کنید
                </button>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {spareParts.map((part) => (
                  <div
                    key={part.id}
                    className="flex items-center justify-between bg-white rounded-xl border border-line p-3 hover:shadow-md transition-shadow group"
                  >
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <div className="min-w-0">
                        <p className="text-sm font-bold text-ink truncate">{part.name}</p>
                        <div className="flex items-center gap-2 text-xs text-muted">
                          <span>{part.price?.toLocaleString()} ریال</span>
                          <span
                            className={`w-1.5 h-1.5 rounded-full ${part.is_active ? 'bg-green-500' : 'bg-zinc-300'}`}
                          />
                          <span className={part.is_active ? 'text-green-600' : 'text-zinc-400'}>
                            {part.is_active ? 'فعال' : 'غیرفعال'}
                          </span>
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center gap-1 flex-shrink-0">
                      <button
                        type="button"
                        onClick={() => onEditProduct({ ...part }, false)}
                        className="p-1.5 rounded-lg hover:bg-brand-soft text-muted hover:text-brand transition-colors"
                        title="ویرایش"
                      >
                        <svg
        ...
        <AddSparePartModal
          ...
          onClose={() => setShowAddSparePart(false)}
          onSaved={() => {
            onSaved();
...
function AddSparePartModal({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      fd.append('is_spare_part', 'true');
      fd.append('main_product', String(productId));
      fd.append('show_in_products', 'true');
          ...
          <div className="space-y-3">
            <div>
              <label className="block text-sm font-bold text-ink-soft mb-1">
                نام قطعه <span className="text-brand">*</span>
              </label>
              <input
                value={form.name}
                onChange={(e) =>
            ...
            <div>
              <label className="block text-sm font-bold text-ink-soft mb-1">
                اسلاگ <span className="text-muted text-xs">(اختیاری)</span>
              </label>
              <input
                value={form.slug}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-sm font-bold text-ink-soft mb-1">قیمت (ریال)</label>
                <input
                  type="number"
                  value={form.price || ''}
              ...
              <div>
                <label className="block text-sm font-bold text-ink-soft mb-1">موجودی</label>
                <input
                  type="number"
                  value={form.stock || ''}
            ...
            <label className="flex items-center gap-3 cursor-pointer p-3 rounded-xl border border-line hover:bg-surface transition-colors">
              <input
                type="checkbox"
              ...
              />
              <span className="text-sm font-bold text-ink">فعال</span>
              <span className="text-xs text-muted mr-auto">قطعه در سایت نمایش داده شود</span>
            </label>

            {error && (
              <p className="text-brand bg-brand-soft rounded-xl px-4 py-2 text-sm">{error}</p>
            )}
            ...
            >
              ...
              {saving ? 'در حال ذخیره...' : 'افزودن قطعه'}
            </button>
          </div>
        </div>
      </div>
...
function CategoryDrawer({
  ...
  async function save() {
    ...
    try {
      ...
      if (isNew) {
        ...
        await apiFetch(`/categories/${form.id}/`, { method: 'PATCH', body: fd, auth: true });
      }
      onSaved();
    } catch (err) {
      setError(err instanceof Error ? err.message : 'خطا در ذخیره');
  ...
  async function remove() {
    ...
    setDeleting(true);
    try {
      await apiFetch(`/categories/${form.id}/`, { method: 'DELETE', auth: true });
      onSaved();
    } catch (err) {
      setError(err instanceof Error ? err.message : 'خطا در حذف');
    } finally {
