...
const EMPTY_SPARE_PART: Partial<Product> = {
  name: '',
  slug: '',
  short_description: '',
  description: '',
...
export default function ProductsAdmin() {
  ...
  const [tab, setTab] = useState<'products' | 'categories' | 'spare-parts'>('products');
  ...
  const [selectedSparePart, setSelectedSparePart] = useState<Partial<Product> | null>(null);
  const [isNewSparePart, setIsNewSparePart] = useState(false);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [p, sp, c] = await Promise.all([
        apiFetch<{ results: Product[] }>('/products/?is_spare_part=false&ordering=-created_at', {
          auth: true,
        }),
        apiFetch<{ results: Product[] }>('/products/?is_spare_part=true&ordering=-created_at', {
          auth: true,
        }),
        apiFetch<Category[]>('/categories/', { auth: true }),
      ]);
      setProducts(p.results ?? []);
      setSpareParts(sp.results ?? []);
  ...
  const currentList = tab === 'products' ? products : tab === 'spare-parts' ? spareParts : cats;
  ...
  async function bulkDelete() {
    ...
    try {
      const endpoint = tab === 'categories' ? 'categories' : 'products';
      const items = tab === 'products' ? products : tab === 'spare-parts' ? spareParts : cats;
      await Promise.all(
        items
  ...
  const getTabTitle = () => {
    if (tab === 'products') return 'محصولات';
    if (tab === 'spare-parts') return 'قطعات یدکی';
    return 'دسته‌بندی‌ها';
  };

  const getTabCount = () => {
    if (tab === 'products') return products.length;
    if (tab === 'spare-parts') return spareParts.length;
    return cats.length;
  };
  ...
  return (
    <div>
      <div className="flex flex-wrap items-start justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-black text-ink mb-1">مدیریت کاتالوگ</h1>
          <p className="text-muted">مدیریت محصولات، قطعات یدکی و دسته‌بندی‌ها</p>
        </div>
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button
            onClick={() => {
              if (tab === 'products') {
                ...
                setSelectedSparePart({ ...EMPTY_SPARE_PART });
                setIsNewSparePart(true);
              } else {
          ...
          >
            <Plus size={18} /> افزودن {getTabTitle().slice(0, -1)}
          </button>
        </div>
      </div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        ...
        <button
          ...
          className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold transition-colors ${tab === 'spare-parts' ? 'bg-brand text-white' : 'bg-white border border-line text-ink-soft'}`}
        >
          <Wrench size={16} /> قطعات یدکی ({spareParts.length})
        </button>
        <button
          onClick={() => {
            setTab('categories');
      ...
      {selectedSparePart !== null && (
        <SparePartDrawer
          product={selectedSparePart}
          isNew={isNewSparePart}
          categories={cats}
          products={products}
          onClose={() => {
          ...
          {products.map((p) => {
            ...
            return (
              ...
              >
                ...
                <td className="p-3 hidden sm:table-cell" onClick={() => onEdit(p)}>
                ...
                <td className="p-3 font-bold text-ink" onClick={() => onEdit(p)}>
                  {p.name}
                </td>
                <td className="p-3 text-muted hidden sm:table-cell" onClick={() => onEdit(p)}>
                  {p.category_name || '—'}
                </td>
                <td className="p-3 font-bold" onClick={() => onEdit(p)}>
                  {p.discount_price > 0 ? (
                    <div>
                      <span className="text-xs line-through text-muted">
                        {p.price.toLocaleString()}
                      </span>
                      <span className="text-brand block">{p.discount_price.toLocaleString()}</span>
                    </div>
                  ) : (
                ...
                <td className="p-3" onClick={() => onEdit(p)}>
                  <span
  ...
  if (spareParts.length === 0) {
    return (
      <div className="text-center py-20 bg-white rounded-3xl border border-line">
        ...
        <p className="text-muted mb-4">هنوز قطعه یدکی ثبت نشده است.</p>
        <button onClick={onAdd} className="btn btn-primary text-sm">
          <Plus size={18} /> اولین قطعه یدکی را اضافه کنید
        </button>
      </div>
    );
  }
      ...
      <table className="w-full text-sm">
        <thead className="bg-surface text-muted">
          <tr>
            ...
            <th className="text-right p-3 font-bold hidden sm:table-cell">تصویر</th>
            <th className="text-right p-3 font-bold">نام قطعه</th>
            <th className="text-right p-3 font-bold hidden sm:table-cell">محصول اصلی</th>
            <th className="text-right p-3 font-bold">قیمت</th>
            <th className="text-right p-3 font-bold hidden sm:table-cell">موجودی</th>
            <th className="text-right p-3 font-bold">وضعیت</th>
          </tr>
        ...
        <tbody>
          {spareParts.map((p) => {
            const src = imgSrc(p.image);
            const mainProduct = products.find((prod) => prod.id === p.main_product);
            return (
              <tr
                key={p.id}
                className={`border-t border-line hover:bg-surface cursor-pointer ${selectedIds.has(p.id) ? 'bg-brand-soft' : ''}`}
        ...
        <tbody>
          {categories.map((c) => (
            ...
            >
              ...
              <td className="p-3 font-bold text-ink" onClick={() => onEdit(c)}>
                {c.name}
              </td>
              <td
              ...
              <td className="p-3 text-muted hidden sm:table-cell" onClick={() => onEdit(c)}>
                {c.order}
              </td>
              <td className="p-3" onClick={() => onEdit(c)}>
                <span
  ...
  async function save() {
    ...
    try {
      ...
      if (isNew) {
        ...
        await apiFetch(`/products/${form.slug}/`, { method: 'PATCH', body: fd, auth: true });
      }
      onSaved();
    } catch (err) {
      ...
      <div className="absolute inset-y-0 right-0 w-full sm:max-w-lg bg-white overflow-y-auto p-5 sm:p-7 shadow-2xl flex flex-col gap-5">
        <div className="flex items-center justify-between">
          <h3 className="text-xl font-black text-ink flex items-center gap-2">
            ...
            {isNew ? 'قطعه یدکی جدید' : 'ویرایش قطعه یدکی'}
          </h3>
          <button onClick={onClose} className="text-muted hover:text-brand">
            <X />
          </button>
        ...
        <div>
          <label className="block text-sm font-bold text-ink-soft mb-2">تصویر قطعه</label>
          <label className="cursor-pointer block">
            <div
              className={`rounded-2xl border-2 border-dashed border-line overflow-hidden flex items-center justify-center bg-surface ${imagePreview ? 'h-52' : 'h-36'} hover:border-brand transition-colors`}
            >
        ...
        <div>
          <label className="block text-sm font-bold text-ink-soft mb-2">نام قطعه *</label>
          <input
            value={form.name || ''}
            onChange={(e) => {
              set('name', e.target.value);
        ...
        >
          <div>
            ...
            <p className="text-xs text-muted mt-0.5">این قطعه در صفحه محصولات نیز نمایش داده شود</p>
          </div>
          <input
            type="checkbox"
        ...
        >
          <div>
            <p className="font-bold text-ink text-sm">✓ قطعه فعال</p>
            <p className="text-xs text-muted mt-0.5">قطعه فعال در سایت نمایش داده می‌شود</p>
          </div>
          <input
            type="checkbox"
            checked={form.is_active !== false}
