...
function AgencyCard({ ag }: { ag: Agency }) {
  return (
    <div className="p-4">
      <div className="font-bold text-ink text-sm mb-2 flex items-center gap-2">
        <Building2 size={14} className="text-brand shrink-0" />
        {ag.name}
        {ag.code && <span className="text-xs text-muted font-normal">({ag.code})</span>}
      </div>
      <p className="text-xs text-muted leading-6 mb-1">
...
function AgencyTypeSection({
  ...
  return (
    <div className="bg-white rounded-3xl border border-line overflow-hidden mb-3">
      <div className={`px-5 py-3 flex items-center gap-2 ${color}`}>
        <Icon size={15} className="text-white" />
        <h4 className="font-black text-white text-sm">{title}</h4>
        <span className="mr-auto text-white/70 text-xs">{agencies.length} نمایندگی</span>
      </div>
      <div className="divide-y divide-line">
        {agencies.map((ag) => (
          <AgencyCard key={ag.id} ag={ag} />
        ))}
      </div>
    </div>
  );
...
function SalesRequestForm() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setError('');
    try {
      const res = await fetch(`${API_BASE}/agency-requests/sales/`, {
        method: 'POST',
  ...
  if (done)
    return (
      <div className="flex flex-col items-center justify-center py-16 gap-4 text-center">
        ...
        <p className="font-black text-ink text-lg">درخواست شما با موفقیت ثبت شد!</p>
        <p className="text-muted text-sm">کارشناسان ما در اسرع وقت با شما تماس می‌گیرند.</p>
      </div>
    );
  ...
  return (
    <form onSubmit={submit} className="grid sm:grid-cols-2 gap-4">
      <div>
        <label className="block text-sm font-bold text-ink-soft mb-2">نام و نام خانوادگی *</label>
        <input
          required
      ...
      <div>
        <label className="block text-sm font-bold text-ink-soft mb-2">نام فروشگاه / گالری *</label>
        <input
          required
      ...
      <div>
        <label className="block text-sm font-bold text-ink-soft mb-2">تلفن ثابت</label>
        <input
      ...
      <div>
        <label className="block text-sm font-bold text-ink-soft mb-2">تلفن همراه *</label>
        <input
          required
      ...
      <div>
        <label className="block text-sm font-bold text-ink-soft mb-2">وضعیت مالکیت فروشگاه *</label>
        <select
          required
      ...
      <div>
        <label className="block text-sm font-bold text-ink-soft mb-2">
          سابقه فعالیت در صنف لوازم خانگی (سال)
        </label>
        <input
          type="number"
          min={0}
      ...
      <div className="sm:col-span-2">
        <label className="block text-sm font-bold text-ink-soft mb-2">آدرس دقیق فروشگاه *</label>
        <textarea
          required
      ...
      {error && (
        <p className="sm:col-span-2 text-brand bg-brand-soft rounded-xl px-4 py-3 text-sm">
          {error}
        </p>
      )}
      <div className="sm:col-span-2">
        ...
        >
          <Send size={16} /> {sending ? 'در حال ارسال...' : 'ارسال درخواست نمایندگی فروش'}
        </button>
      </div>
    </form>
  );
...
function ServiceRequestForm() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setError('');
    try {
      const res = await fetch(`${API_BASE}/agency-requests/service/`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(form),
      });
  ...
  return (
    <form onSubmit={submit} className="grid sm:grid-cols-2 gap-4">
      ...
      <div>
        <label className="block text-sm font-bold text-ink-soft mb-2">
          مدرک تحصیلی یا فنی‌وحرفه‌ای
        </label>
        <input
          value={form.education}
          onChange={(e) => set('education', e.target.value)}
          className={field}
          placeholder="مثال: کارشناس برق / مدرک فنی‌وحرفه‌ای"
        />
      </div>
      <div>
        <label className="block text-sm font-bold text-ink-soft mb-2">
          تعداد تکنسین‌های فعال *
        </label>
        <input
          type="number"
          min={1}
          ...
          onChange={(e) => set('technician_count', parseInt(e.target.value) || 1)}
          className={field}
        />
      </div>
      <div className="sm:col-span-2">
        <label className="block text-sm font-bold text-ink-soft mb-2">
          سابقه همکاری با برندهای دیگر
        </label>
        <input
          value={form.previous_brands}
          onChange={(e) => set('previous_brands', e.target.value)}
          className={field}
          placeholder="مثال: سامسونگ، ال‌جی، بوش"
        />
      </div>
      <div className="sm:col-span-2">
        <label className="block text-sm font-bold text-ink-soft mb-2">امکانات موجود</label>
        <textarea
          value={form.facilities}
          onChange={(e) => set('facilities', e.target.value)}
          rows={3}
          className={field}
          placeholder="مثال: پیک تحویل، فضای انبار قطعات، تجهیزات تعمیر"
        />
      </div>
      {error && (
        <p className="sm:col-span-2 text-brand bg-brand-soft rounded-xl px-4 py-3 text-sm">
      ...
      <div className="sm:col-span-2">
        <button
          ...
          className="btn btn-primary w-full justify-center gap-2"
        >
          <Send size={16} /> {sending ? 'در حال ارسال...' : 'ارسال درخواست نمایندگی خدمات'}
        </button>
      </div>
    </form>
  );
...
export default function AgenciesClient({ agencies }: { agencies: Agency[] }) {
  const [selected, setSelected] = useState<string | null>(null);
  const [activeForm, setActiveForm] = useState<'sales' | 'service' | null>(null);
  const provincesWithAgency = new Set(agencies.map((a) => a.province));
  const selectedAgencies = selected ? agencies.filter((a) => a.province === selected) : [];
  const salesAgencies = selectedAgencies.filter((a) => a.agency_type === 'sales');
  const serviceAgencies = selectedAgencies.filter((a) => a.agency_type === 'service');
    ...
    <div className="container-x py-10 mt-20">
      <div className="grid lg:grid-cols-[1.6fr_1fr] gap-6 items-start">
        ...
        <div className="bg-white rounded-3xl border border-line p-4">
          <IranSvgMap
            ...
            hasAgency={(id) => provincesWithAgency.has(id)}
          />
        ...
        <div className="flex flex-col gap-4">
          {selected && (
            <div>
              <div className="bg-brand px-5 py-3 rounded-t-3xl flex items-center justify-between">
                <h3 className="font-black text-white text-sm flex items-center gap-2">
                  <MapPin size={16} /> نمایندگان استان {selectedProvinceName}
                </h3>
                <button
              ...
              {selectedAgencies.length === 0 ? (
                <div className="p-6 text-center text-muted text-sm bg-white rounded-b-3xl border border-line border-t-0">
                  در این استان نمایندگی ثبت نشده است.
                </div>
              ) : (
                <div className="bg-surface rounded-b-3xl border border-line border-t-0 p-3">
                  <AgencyTypeSection
                    title="نمایندگی فروش"
                    icon={Store}
                    color="bg-[#1a5276]"
                    agencies={salesAgencies}
                  />
                  <AgencyTypeSection
                    title="نمایندگی خدمات پس از فروش و گارانتی"
                    icon={Wrench}
                    color="bg-[#6c3483]"
                    agencies={serviceAgencies}
                  />
                </div>
              )}
            </div>
          ...
          <div className="bg-white rounded-3xl border border-line overflow-hidden">
            <div className="bg-[#2c3e50] px-5 py-3 flex items-center gap-4">
              <h3 className="font-black text-white text-sm">استان‌‌ها</h3>
              <div className="flex items-center gap-3 mr-auto text-xs text-white/60">
                <span className="flex items-center gap-1">
                  <span className="w-3 h-3 rounded bg-[#1a3a5c] inline-block"></span> دارای نمایندگی
                </span>
              </div>
            </div>
            <div className="grid grid-cols-2">
              {ALL_PROVINCES.map((prov) => {
                const has = provincesWithAgency.has(prov.id);
                const isSel = selected === prov.id;
                return (
                  <button
                    key={prov.id}
                    onClick={() => setSelected(isSel ? null : prov.id)}
                    className={`flex items-center justify-between px-4 py-2.5 text-sm border-b border-l border-line transition-colors text-right
                      ${isSel ? 'bg-brand text-white font-bold' : has ? 'bg-[#1a3a5c] text-white hover:bg-[#1e4a70]' : 'bg-[#2c3e50] text-white/70 hover:bg-[#354a5e]'}`}
                  >
                    <span>{prov.name}</span>
                    {isSel && <ChevronLeft size={13} />}
                  </button>
                );
              })}
            </div>
      ...
      <div className="mt-16">
        <div className="text-center mb-10">
          <h2 className="text-2xl md:text-3xl font-black text-ink mb-3">اخذ نمایندگی بلانزو</h2>
          <p className="text-muted max-w-xl mx-auto">
            اگر متقاضی دریافت نمایندگی فروش یا خدمات پس از فروش بلانزو هستید، فرم مربوطه را تکمیل
            کنید.
          </p>
        </div>
        ...
        <div className="flex gap-3 mb-6 justify-center">
          <button
            onClick={() => setActiveForm(activeForm === 'sales' ? null : 'sales')}
            className={`flex items-center gap-2 px-6 py-3 rounded-2xl font-bold text-sm transition-all border ${activeForm === 'sales' ? 'bg-[#1a5276] text-white border-[#1a5276]' : 'bg-white text-ink border-line hover:border-[#1a5276]'}`}
          >
            <Store size={16} /> نمایندگی فروش
          </button>
          <button
            onClick={() => setActiveForm(activeForm === 'service' ? null : 'service')}
            className={`flex items-center gap-2 px-6 py-3 rounded-2xl font-bold text-sm transition-all border ${activeForm === 'service' ? 'bg-[#6c3483] text-white border-[#6c3483]' : 'bg-white text-ink border-line hover:border-[#6c3483]'}`}
          >
        ...
        {activeForm === 'sales' && (
          <div className="bg-white rounded-3xl border border-line p-6 md:p-10 max-w-3xl mx-auto">
            <div className="flex items-center gap-3 mb-6 pb-4 border-b border-line">
              <span className="grid place-items-center w-10 h-10 rounded-xl bg-[#1a5276]/10 text-[#1a5276]">
                <Store size={20} />
              </span>
              <div>
                <h3 className="font-black text-ink">فرم نمایندگی فروش</h3>
                <p className="text-xs text-muted">درخواست اخذ نمایندگی فروش محصولات بلانزو</p>
              </div>
            </div>
            <SalesRequestForm />
          </div>
        ...
        {activeForm === 'service' && (
          <div className="bg-white rounded-3xl border border-line p-6 md:p-10 max-w-3xl mx-auto">
            <div className="flex items-center gap-3 mb-6 pb-4 border-b border-line">
              <span className="grid place-items-center w-10 h-10 rounded-xl bg-[#6c3483]/10 text-[#6c3483]">
                <Wrench size={20} />
              </span>
              <div>
                <h3 className="font-black text-ink">فرم نمایندگی خدمات پس از فروش</h3>
                <p className="text-xs text-muted">درخواست اخذ نمایندگی خدمات و تعمیرات بلانزو</p>
              </div>
            </div>
            <ServiceRequestForm />
          </div>
...
function IranSvgMap({
  ...
  const fill = (id: string) =>
    selected === id ? '#c0392b' : hasAgency(id) ? '#1a5276' : '#7f8c8d';
  ...
  return (
    ...
    >
      ...
      <g>
        ...
        <path
          id="azerbaijan-east"
          fill={fill('azerbaijan-east')}
          stroke="#ecf0f1"
          strokeWidth="1.5"
          className="cursor-pointer transition-colors duration-150 hover:brightness-110"
          onClick={() => onSelect('azerbaijan-east')}
        ...
        <path
          id="azerbaijan-west"
          fill={fill('azerbaijan-west')}
          stroke="#ecf0f1"
          strokeWidth="1.5"
          className="cursor-pointer transition-colors duration-150 hover:brightness-110"
          onClick={() => onSelect('azerbaijan-west')}
        ...
        <path
          id="chahar-mahaal-bakhtiari"
          fill={fill('chahar-mahaal-bakhtiari')}
          stroke="#ecf0f1"
          strokeWidth="1.5"
          className="cursor-pointer transition-colors duration-150 hover:brightness-110"
          onClick={() => onSelect('chahar-mahaal-bakhtiari')}
        ...
        <path
          id="kermanshah"
          fill={fill('kermanshah')}
          stroke="#ecf0f1"
          strokeWidth="1.5"
          className="cursor-pointer transition-colors duration-150 hover:brightness-110"
          onClick={() => onSelect('kermanshah')}
        ...
        <path
          id="khorasan-north"
          fill={fill('khorasan-north')}
          stroke="#ecf0f1"
          strokeWidth="1.5"
          className="cursor-pointer transition-colors duration-150 hover:brightness-110"
          onClick={() => onSelect('khorasan-north')}
        ...
        <path
          id="khorasan-razavi"
          fill={fill('khorasan-razavi')}
          stroke="#ecf0f1"
          strokeWidth="1.5"
          className="cursor-pointer transition-colors duration-150 hover:brightness-110"
          onClick={() => onSelect('khorasan-razavi')}
        ...
        <path
          id="khorasan-south"
          fill={fill('khorasan-south')}
          stroke="#ecf0f1"
          strokeWidth="1.5"
          className="cursor-pointer transition-colors duration-150 hover:brightness-110"
          onClick={() => onSelect('khorasan-south')}
        ...
        <path
          ...
          fill={fill('kohgiluyeh-boyer-ahmad')}
          stroke="#ecf0f1"
          strokeWidth="1.5"
          className="cursor-pointer transition-colors duration-150 hover:brightness-110"
          onClick={() => onSelect('kohgiluyeh-boyer-ahmad')}
        ...
        <path
          id="mazandaran"
          fill={fill('mazandaran')}
          stroke="#ecf0f1"
          strokeWidth="1.5"
          className="cursor-pointer transition-colors duration-150 hover:brightness-110"
          onClick={() => onSelect('mazandaran')}
        ...
        <path
          ...
          fill={fill('sistan-baluchestan')}
          stroke="#ecf0f1"
          strokeWidth="1.5"
          className="cursor-pointer transition-colors duration-150 hover:brightness-110"
          onClick={() => onSelect('sistan-baluchestan')}
